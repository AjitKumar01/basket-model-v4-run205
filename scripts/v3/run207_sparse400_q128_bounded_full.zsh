...
# Exact continuation of run205 from its best/recovery checkpoint at iteration 600.
# Statistical model, support, optimizer, scheduler and RNG are unchanged.  The estimator
# uses the frozen-audited Q128 base and the bounded sparse-polynomial adjoint; zero-centred
# RQMC removes an unnecessary proposal backward while lambda_max is in the unique-mode
# regime.  Independent retries and Q512 checkpoint evaluation retain accuracy guards.
env V3_AFFINITY=1 python scripts/v3/fit.py \
  --label run207_sparse400_q128_bounded_full \
  ...
  --qmc-n 128 --qmc-reps 4 --qmc-refresh-every 1 --qmc-eval-n 512 \
  ...
  2>&1 | tee out/v3_run207_sparse400_q128_bounded_full.log
